...
export default function SiteBuilderPage() {
  ...
  async function handleResearch() {
    ...
    addLog(`Tavily: מחפש "${researchQuery.slice(0, 40)}..."...`);
    try {
  ...
  async function handleMedia() {
    ...
    addLog("Nano Banana: מייצר מדיות...");
  ...
  function viewBtn(mode: PreviewMode, hasData: boolean) {
    ...
    return (
      <button
        className={`btn ${previewMode === mode ? "btn-primary" : "btn-secondary"}`}
        style={{ padding: "0.2rem 0.5rem", fontSize: "0.7rem" }}
        onClick={() => {
          setPreviewMode(mode);
          if (mode === "site" && iframeRef.current && generatedHtml) {
            iframeRef.current.src = URL.createObjectURL(new Blob([generatedHtml], { type: "text/html" }));
          }
        }}
      >
        הצג
      ...
      return (
        <div className="preview-data">
          ...
          <div className="data-section">
            <div className="data-label">כותרת</div>
            <div className="data-value">{String(scrapeData.title ?? "—")}</div>
          </div>
          <div className="data-section">
            <div className="data-label">תיאור</div>
            <div className="data-value">{String(scrapeData.description ?? "—")}</div>
          </div>
          <div className="data-section">
            <div className="data-label">כתובת מקור</div>
            <div className="data-value" dir="ltr">{String(scrapeData.sourceUrl ?? "—")}</div>
          </div>
          <div className="data-section">
            <div className="data-label">תוכן (Markdown)</div>
            <pre className="data-pre">{String(scrapeData.markdown ?? "").slice(0, 5000)}</pre>
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="preview-data">
          ...
          {typeof researchData.answer === "string" && researchData.answer && (
            <div className="data-section">
              <div className="data-label">סיכום</div>
              <div className="data-value" style={{ lineHeight: 1.7 }}>{String(researchData.answer)}</div>
            </div>
          )}
          <div className="data-section">
              ...
              <div key={i} className="media-card">
                {m.imageUrl ? (
                  <>
                    ...
                    <div className="media-card-footer">
                      ...
                      <button className="btn btn-secondary btn-sm" onClick={() => {
                        const a = document.createElement("a");
                        a.href = String(m.imageUrl);
                        a.download = `media-${i + 1}.png`;
                        a.click();
                      }}>הורד</button>
                    </div>
                  </>
                  ...
                  <div className="media-error">
                    <span>✗</span>
                    <span style={{ fontSize: "0.75rem" }}>{String(m.error ?? "שגיאה")}</span>
                  </div>
                )}
    ...
    <div className="app-layout">
      ...
      <div className="form-panel" style={{ width: `${formWidth}%` }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <span style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>סרוק → חקור → מדיה → בנה</span>
        </div>
        ...
        <div className="form-group">
          <div className="step-header">
            <div className="step-title"><span className="step-number">1</span>סריקת אתר (Firecrawl)</div>
            <div style={{ display: "flex", gap: "0.3rem", alignItems: "center" }}>
              {viewBtn("scrape", !!scrapeData)}
              {badge(scrapeStatus)}
            </div>
          </div>
          <div style={{ display: "flex", gap: "0.4rem" }}>
            <input id="url" type="url" className="form-input" placeholder="https://example.com" value={url} onChange={(e) => setUrl(e.target.value)} dir="ltr" style={{ flex: 1 }} />
            <button className="btn btn-primary" onClick={handleScrape} disabled={!url || scrapeStatus === "loading"}>
              {scrapeStatus === "loading" ? <span className="spinner" /> : "סרוק"}
            </button>
          </div>
        </div>
        ...
        <div className="form-group">
          <div className="step-header">
            <div className="step-title"><span className="step-number">2</span>מחקר (Tavily)</div>
            <div style={{ display: "flex", gap: "0.3rem", alignItems: "center" }}>
              {viewBtn("research", !!researchData)}
              {badge(researchStatus)}
            </div>
          </div>
          ...
          <button className="btn btn-primary" onClick={handleResearch} disabled={!researchQuery || researchStatus === "loading"} style={{ alignSelf: "flex-start" }}>
            {researchStatus === "loading" ? <span className="spinner" /> : "חקור"}
          </button>
        </div>
        ...
        <div className="form-group">
          <div className="step-header">
            <div className="step-title"><span className="step-number">3</span>יצירת מדיות (Nano Banana)</div>
            <div style={{ display: "flex", gap: "0.3rem", alignItems: "center" }}>
              {viewBtn("media", !!mediaData)}
              {badge(mediaStatus)}
            </div>
          </div>
          ...
          <button className="btn btn-primary" onClick={handleMedia} disabled={!mediaPrompt || mediaStatus === "loading"} style={{ alignSelf: "flex-start" }}>
            {mediaStatus === "loading" ? <span className="spinner" /> : "צור מדיות"}
          </button>
        </div>
        ...
        <div className="form-group">
          <div className="step-header">
            <div className="step-title"><span className="step-number">4</span>הפקת אתר (Claude AI)</div>
            <div style={{ display: "flex", gap: "0.3rem", alignItems: "center" }}>
              {viewBtn("site", !!generatedHtml)}
              {badge(generateStatus)}
            </div>
          </div>
          ...
          <button className="btn btn-generate" onClick={handleGenerate} disabled={!canGenerate} style={{ width: "100%" }}>
            {isGenerating ? <><span className="spinner" /> מייצר אתר...</> : "בנה אתר"}
          </button>
      ...
      <div className="preview-panel">
        <div className="preview-header">
          ...
          <div style={{ display: "flex", gap: "0.3rem" }}>
            {generatedHtml && (
              <button className="btn btn-secondary btn-sm" onClick={() => downloadFile(generatedHtml, `${siteName || "site"}.html`, "text/html")}>
                הורד HTML
              </button>
            )}
          </div>
